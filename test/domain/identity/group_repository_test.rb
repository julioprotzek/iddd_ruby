...
class GroupRepositoryTest < IdentityAccessTest
  setup do
  ...
  test 'remove group referenced user' do
    ...
    re_grouped = DomainRegistry.group_repository.group_named(tenant.tenant_id, 'Group A')
    assert_equal 'Group A', re_grouped.name
  ...
  test 'remove repository group' do
    tenant = tenant_aggregate
    group_a = tenant.provision_group(name: 'Group A', description: 'A group named Group A')
    DomainRegistry.group_repository.add(group_a)
    not_nil_group = DomainRegistry.group_repository.group_named(tenant.tenant_id, 'Group A')
    assert_not_nil not_nil_group

    DomainRegistry.group_repository.remove(group_a)
    nil_group = DomainRegistry.group_repository.group_named(tenant.tenant_id, 'Group A')
